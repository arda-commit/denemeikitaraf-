...
exports.run = async (client, message, args) => {
  ...
  const Durum = user.presence.status;
    ...
    const embed = new Discord.MessageEmbed()
		...
		.addField("» Rolleri:", `${member.roles.cache.filter(r => r.name !== "@everyone").map(r => r).join(' **|** ') ? member.roles.filter(r => r.name !== "@everyone").map(r => r).join(' **|** ') : 'Bu kullanıcının bu sunucuda hiç rolü bulunmuyor.'}`)
