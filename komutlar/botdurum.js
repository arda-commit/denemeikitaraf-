...
exports.run = (client, message, args) => {
    if(!args[0]) return message.channel.send(new Discord.MessageEmbed().setColor('BLACK').setDescription(`Bir komut belirtmen gerekli!`))
    if(args[0] === "oynanıyor"){
      var text = args.slice(1).join(' ')
        if(!text) return message.channel.send(new Discord.MessageEmbed().setColor('BLACK').setDescription(`Bir durum belirtmen gerekli! Eğer kullanımı bilmiyorsan `))
